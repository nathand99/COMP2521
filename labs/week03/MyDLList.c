...
void DLListBefore(DLList L, char *it)
{
	...
	// if curr is the first node add new on the start
	...
	// if curr is in the middle, add it in
	else {
...
void DLListAfter(DLList L, char *it)
{
    ...
    // if curr is the last node append new on the end
...
// if current was last, prev becomes new last
// if current was only item, current becomes null
void DLListDelete(DLList L)
{
	assert (L != NULL);
	
	// if curr is the only item, make curr -> NULL
	if (L -> curr -> prev == NULL && L -> curr -> next == NULL) {
	    L -> curr = NULL;
	    free(L -> curr);
	}
	
	// if curr is first, curr -> next is first
	else if (L -> curr == L -> first) {
	    L -> first = L -> curr -> next;
	...
	// if curr is last, curr -> previous is last
	else if (L -> curr == L -> last) {
	...
	// if item is in the middle, link nodes left and right of curr
	else {
