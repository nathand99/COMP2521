...
//The function should fail if the original list (before insertion) is not sorted. Don't
//forget to handle the cases of (a) empty list, (b) smallest value, (c) largest value, (d)
//second-smallest value, (e) second-largest value, (f) value somewhere in the middle
...
void IntListInsertInOrder(IntList L, int v)
{
    ...
    // if list given is not sorted, function should fail
	if (!(IntListIsSorted(L))) {
	    printf("List given isn't sorted");
	    exit(1);
	}
	
	// we are always adding a number to the list so size always +1
	L -> size = L -> size + 1;
	...
	// v is less than the first term
	else if (v < head -> data) {
	    L -> first = new;
	    new -> next = head;
	}
	
	// if v fits in the middle or the end, find where it goes and add it in
	else {
	    prev = curr;
	    curr = curr -> next;
