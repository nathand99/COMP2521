...
int main(int argc, char **argv)
{
	...
	if (id1 == id2) {
        printf("Please make locations different to eachother!");
    }
	//////////////////////////
	if (n == 0)
...
static void runtests (void) {
    ...
    assert(connections(europe, LONDON, MANCHESTER, t) == 2);
    
    assert(connections(europe, LIVERPOOL, DUBLIN, t) == 1);
    assert(t[0] == BOAT);
    
    assert(connections(europe, LISBON, MADRID, t) == 2);
    
    assert(connections(europe, ROME, NAPLES, t) == 3);
    
    assert(connections(europe, PARIS, GENEVA, t) == 1);
    assert(t[0] == ROAD);
    
    assert(connections(europe, VIENNA, BUDAPEST, t) == 2);
