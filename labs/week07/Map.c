...
int connections(Map g, LocationID start, LocationID end, TransportID type[])
{
 ...
 //  int i = 0;
   int type_counter = 0;
   //showMap(g);
   while (g->connections[start] != NULL) {     
       if (g->connections[start]->v == end) {
           type[type_counter] = g->connections[start]->type;
           type_counter++;
       }       
       g->connections[start] = g->connections[start]->next;
   }
   
   if (searchForBoatConnection (start,end) == 1) {
		type[type_counter] = BOAT;
		type_counter++;	
	}
...
static int searchForBoatConnection (LocationID start, LocationID end) {
    ...
    if ((start == ROME || start == NAPLES || start == CAGLIARI || start == GENOA)) {

        if ((end == ROME || end == NAPLES || end == CAGLIARI || end == GENOA)) {
	        printf("Both Cities are Port Cities in Tyrrhenian Sea\n");
	        result = 1;
        }
        
    } else if ((start == VENICE || start == BARI)) {

        if ((end == VENICE || end == BARI)) {
	        printf("Both Cities are Port Cities in Adriatic Sea\n");
	        result = 1;
        }
        
    } else if ((start == VALONA || start == ATHENS || start == SALONICA)) {

        if ((end == VALONA || end == ATHENS || end == SALONICA)) {
	        printf("Both Cities are Port Cities in Ionian Sea\n");
            result = 1;
        }

    } else if ((start == CONSTANTA || start == VARNA)) {
    
        if ((end == CONSTANTA || end == VARNA)) {
            printf("Both Cities are Port Cities in Black Sea\n");
            result = 1;
        }
        
    } else if ((start == CAGLIARI || start == ALICANTE || start == BARCELONA || start == MARSEILLES)) {
    
        if ((end == CAGLIARI || end == ALICANTE || end == BARCELONA || end == MARSEILLES)) {
            printf("Both Cities are Port Cities in Mediterranean Sea\n");
            result = 1;
        }
    } else if ((start == LISBON || start == CADIZ || start == GALWAY)) {

        if ((end == LISBON || end == CADIZ || end == GALWAY)) {
	        printf("Both Cities are Port Cities in Atlantic Ocean\n");
            result = 1;
        }

    } else if ((start == DUBLIN || start == LIVERPOOL || start == SWANSEA)) {

        if ((end == DUBLIN || end == LIVERPOOL || end == SWANSEA)) {
	        printf("Both Cities are Port Cities in Irish Sea\n");
            result = 1;
        }

    } else if ((start == PLYMOUTH || start == LONDON || start == LE_HAVRE)) {

        if ((end == PLYMOUTH || end == LONDON || end == LE_HAVRE)) {
	        printf("Both Cities are Port Cities in English Channel\n");
            result = 1;
        }

    }  else if ((start == SANTANDER || start == BORDEAUX || start == NANTES)) {

        if ((end == SANTANDER || end == BORDEAUX || end == NANTES)) {
	        printf("Both Cities are Port Cities in Bay Of Biscay\n");
            result = 1;
        }

    }   else if ((start == EDINBURGH || start == AMSTERDAM || start == HAMBURG)) {

        if ((end == EDINBURGH || end == AMSTERDAM || end == HAMBURG)) {
